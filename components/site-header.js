...
export default function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-20 border-b border-[var(--line)] bg-white/88 backdrop-blur">
      <div className="site-shell">
        <div className="relative flex min-h-[76px] items-center justify-between gap-4">
          ...
          <button aria-controls="site-nav" aria-expanded={open} aria-label="Toggle navigation" className="inline-flex size-12 items-center justify-center rounded-2xl border border-[var(--line)] bg-white md:hidden" onClick={() => setOpen((value) => !value)} type="button">
          ...
          <nav className={`${open ? "flex" : "hidden"} absolute left-0 right-0 top-[calc(100%+0.5rem)] flex-col gap-3 rounded-[28px] border border-[var(--line)] bg-white p-4 shadow-[0_24px_60px_rgba(15,23,42,0.08)] md:static md:flex md:flex-row md:items-center md:gap-4 md:rounded-none md:border-0 md:bg-transparent md:p-0 md:shadow-none`} id="site-nav">
            ...
            <a className="cta-primary mt-2 md:ml-3 md:mt-0" href={contactInfo.bookingUrl} onClick={() => setOpen(false)} rel="noopener noreferrer" target="_blank">
              Book Free Session
            </a>
          </nav>
        </div>
